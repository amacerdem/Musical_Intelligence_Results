"""L3 — Engine cache .npz integrity for PMEmo."""
...
def test_aac_cluster_present(engine_cache_dir):
    """AAC mechanism (drives arousal Tier-1 finding) must be in cache."""
...
def test_srp_cluster_present(engine_cache_dir):
    """SRP mechanism (drives valence Tier-1 finding) must be in cache."""
    npz = next(engine_cache_dir.glob("*.npz"))
    z = np.load(npz, allow_pickle=True)
    assert "mech_SRP" in z.files


def test_vmm_cluster_present(engine_cache_dir):
    """VMM mechanism (secondary valence Bonferroni-pass channels) must be in cache."""
    npz = next(engine_cache_dir.glob("*.npz"))
    z = np.load(npz, allow_pickle=True)
    assert "mech_VMM" in z.files


def test_engine_duration_sane(engine_cache_dir):
    """PMEmo chorus extracts are short (~22 s median); engine output should match."""
    ENGINE_HZ = 172.265625
    durations = []
    for p in sorted(engine_cache_dir.glob("*.npz"))[:10]:
        z = np.load(p, allow_pickle=True)
        durations.append(z["r3"].shape[0] / ENGINE_HZ)
    assert min(durations) > 5, "Some clips suspiciously short (<5s)"
    assert max(durations) < 120, "Some clips suspiciously long (>120s)"
