#!/usr/bin/env python3
"""Top-level orchestrator for 23-h4-h5-pmemo-dynamic-emotion.
...
LAYERS = [
    ...
    ("L2", "L2_data_integrity",           "PMEmo CSVs + engine .npz files present"),
    ("L3", "L3_engine_cache",             "Engine cache per-frame integrity (AAC/SRP/VMM clusters)"),
    ("L4", "L4_ceiling_check",            "LOSO ceilings: arousal +0.171 [0.158, 0.184], valence +0.158 [0.144, 0.172]"),
    ("L5", "L5_primary_test",             "PRIMARY — H4 AROUSAL-19 + H5 VALENCE-15 lag-aware Spearman + Bonferroni"),
...
def _write_report(rows, started, worst_rc, aborted=None):
    ...
    body = [
        "# 23-h4-h5-pmemo-dynamic-emotion — Run Report",
        "",
        f"- **Started:**  {started}",
        f"- **Finished:** {finished}",
        f"- **Headline:** {headline}",
        ...
        *rows,
        ...
        "**Arousal (H4):** `MECH_AAC__E0:emotional_arousal` ρ=+0.162, ceiling-rel = 94.7 %.",
        "**Valence (H5):** `MECH_SRP__P0:wanting` ρ=+0.120, p_bonf=0.0038, 3/15 Bonferroni-pass.",
        "**Cross-paradigm replication:** AAC autonomic cluster generalizes from TenseMusic (109%) to PMEmo (94.7%).",
        "",
    ]
